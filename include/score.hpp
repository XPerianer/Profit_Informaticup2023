...
inline profit::ProductCount product_at_turn(const auto& pipeline, Turn empty_at, Turn turn) {
...
inline profit::ProductScore score(const profit::FieldState& state, Turn turn,
  ...
  for (const auto& [pipeline_id, pipeline] : state.pipelines) {
    auto product_count = product_at_turn(pipeline, when_empty[pipeline.deposit_id], turn);
